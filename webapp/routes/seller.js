...
router.get("/", verifyToken, requireSeller, async (req, res) => {
  try {
    ...
    const listings = await Listing.find({ owner: sellerId }).sort({
      createdAt: -1,
    });
...
router.post(
  ...
  async (req, res) => {
    try {
      const { name, category, region, pricePerDay, sellerName, ageInYears } =
        req.body;
      const sellerId = req.user.userId;
      ...
      // If an image was uploaded, run image verification
      if (req.file) {
        const imagePath = path.join(__dirname, "../public", imgPath);
        const pythonScript = path.join(__dirname, "../verify_image.py");

        // Execute Python script to verify image
        const pythonProcess = exec(
          `python "${pythonScript}" "${imagePath}" "${category}"`,
          (error, stdout, stderr) => {
            if (error) {
              console.error("Image verification error:", error);
              return; // Continue without verification if Python fails
            }
            ...
            try {
              ...
              if (result.success) {
                ...
                Listing.findOneAndUpdate(
                  { _id: newListing._id },
                  {
                    "imageVerification.status": result.is_correct
                      ? "verified"
                      : "flagged",
                    "imageVerification.predictedCategory":
                      result.predicted_category,
                    "imageVerification.confidence": result.confidence,
                    "imageVerification.isCorrect": result.is_correct,
                  }
                ).catch((err) =>
                  console.error("Error updating image verification:", err)
                );
              }
            } catch (parseError) {
              console.error(
                "Error parsing image verification result:",
                parseError
              );
            }
          }
...
router.post("/validate-price", (req, res) => {
  ...
  py.on("error", (err) => {
    ...
    res.status(500).json({
      valid: false,
      message: "Server configuration error: Python not found",
    });
  });
  ...
  py.on("close", () => {
    try {
      ...
      console.error(
        "Error parsing validator output:",
        err,
        "Raw output:",
        result
      );
